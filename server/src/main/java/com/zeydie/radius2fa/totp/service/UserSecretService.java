...
import com.zeydie.radius2fa.totp.repository.UserSecretRepository;
...
public class UserSecretService {
    ...
    /*public @NotNull UserSecretEntity buildAndSave(@NonNull final UserLdapEntity userLdapEntity) {
        @Nullable val existedUserSecretEntity = this.findById(userLdapEntity.getId());

        if (existedUserSecretEntity != null)
            return existedUserSecretEntity;

        @Nullable val userSecretEntity = UserLdapMapper.INSTANCE.toUserSecretEntity(userLdapEntity);

        if (userSecretEntity == null)
            throw new NullPointerException(String.format("Cannot build UserSecretEntity from UserLdapEntity(%s)", userLdapEntity));

        return this.userSecretRepository.save(userSecretEntity);
    }*/
    ...
    public void removeSecretTOTP(@NonNull final Name id) {
        @NonNull val userSecretEntity = this.findById(id);
        @NonNull val login = userSecretEntity.getLogin();
        @Nullable val secretTOTP = userSecretEntity.getSecretTOTP();
